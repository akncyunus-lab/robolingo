...
try{
  ...
  if(plugin?.speak){
    await plugin.speak({text,lang:'en-US',rate:0.82,pitch:1.0,volume:1.0});
    return true;
  }
}catch(err){}
return false;
...
try{
  ...
  recognition.onresult=e=>{
    ...
    const target='hello';
    const ok=heard.includes(target)||heard.replace(/[^a-z]/g,'').includes(target);
    if(ok){
      $('sayHint').textContent='Harika söyledin! 🎉';
      toastMsg('Harika! “Hello” doğru duyuldu ⭐');
      setTimeout(()=>step(3),700);
    }else{
      $('sayHint').textContent=`Robo “${heard||'...'}” duydu. Bir daha deneyelim!`;
      toastMsg('Bir daha deneyelim 🤖');
    }
  };
  recognition.onerror=e=>{
    if(e.error==='not-allowed'||e.error==='service-not-allowed')toastMsg('Mikrofon izni verilmedi. Android ayarlarından mikrofon iznini aç.');
    else if(e.error==='network')toastMsg('Konuşma tanıma için internet bağlantısı gerekiyor.');
    else toastMsg('Mikrofon dinleme başlatılamadı.');
  };
  recognition.onend=()=>{
    listening=false;
