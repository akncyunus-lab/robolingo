...
const NativeTTS=nativePlugin('TextToSpeech');
const NativeSpeech=nativePlugin('SpeechRecognition');

async function nativeTTS(text){
try{
  if(NativeTTS?.speak){
    await NativeTTS.speak({text,lang:'en-US',rate:0.82,pitch:1.0,volume:1.0,queueStrategy:0});
    return true;
  }
}catch(err){}
return false;
...
async function speak(text){
  ...
  if(!browserOK)toastMsg('Bu APK sesli konuşmayı desteklemiyor. Android TTS desteği eklenmesi gerekiyor.');
}
...
async function listenAndCheck(){
  if(NativeSpeech?.start){
    try{
      ...
      $('sayHint').textContent='Hello de ve Robo seni dinlesin 🤖';
      const result=await NativeSpeech.start({
        language:'en-US',maxResults:3,prompt:'Say Hello',partialResults:false,popup:false
      });
      const matches=(result?.matches||[]).map(x=>String(x).toLowerCase());
      ...
      if(ok){
        ...
        $('sayHint').textContent=`Robo “${matches[0]||'...'}” duydu. Bir daha deneyelim!`;
        toastMsg('Bir daha deneyelim 🤖');
      }
      return;
    }catch(err){
      if(String(err?.message||err).toLowerCase().includes('permission'))
        toastMsg('Mikrofon izni verilmedi.');
      else
        toastMsg('Mikrofon başlatılamadı.');
    }finally{
      listening=false;
      $('repeatSpeak').textContent='🎤 Hello — Konuş';
    }
    return;
  }
