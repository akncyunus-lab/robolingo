...
shuffleChoices();
// Her yeni girişte önceki cevap/yeşil durumlarını tamamen sıfırla.
document.querySelectorAll('#matchChoices .choice, #answerChoices .answer').forEach(x=>x.classList.remove('selected-correct','selected-wrong'));
if($('matchFeedback')){$('matchFeedback').textContent='';$('matchFeedback').classList.remove('correct');}
if($('questionFeedback')){$('questionFeedback').textContent='';$('questionFeedback').classList.remove('correct');}
...
if(n===2){
  $('sayHint').textContent='Şimdi sen söyle! Mikrofonla konuş ve Robo cevabını kontrol etsin.';
  if($('afterRepeat')) $('afterRepeat').classList.add('hidden');
}
if(n===3){
  $('sayHint').textContent='Doğru resmi seçerek ilerle!';
  if($('afterRepeat')) $('afterRepeat').classList.add('hidden');
}
...
async function listenAndCheck(){
  ...
  if(sr?.start){
    try{
      ...
      const normalized=matches.map(x=>x.replace(/[^a-z]/g,''));
      const ok=normalized.some(x=>x==='hello');
      if(ok){
        $('sayHint').textContent='Harika söyledin! 🎉';
        toastMsg('Harika! “Hello” doğru duyuldu ⭐');
        setTimeout(()=>step(3),700);
      }else{
        $('sayHint').textContent=`Robo “${matches[0]||'bir şey'}” duydu. Bu doğru değil. Tekrar dene!`;
        toastMsg('Yanlış söyledin 🤖 “Hello” demeyi tekrar dene.');
      }
    }catch(err){
  ...
  try{
    ...
    recognition.onresult=e=>{
      ...
      const ok=heard.replace(/[^a-z]/g,'')==='hello';
...
if(b.id==='afterRepeat'){e.preventDefault();return}
